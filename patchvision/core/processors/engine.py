...
class InferenceEngine:
    ...
    def process(self, 
        ...
        # Convert numpy array to list for batching if needed
        if isinstance(inputs, np.ndarray):
            if inputs.ndim == 4:  # Batch dimension exists
        ...
        # Adaptive batching
        if len(input_list) > self.batch_size * 2:
            # Use dynamic batching
            batches = self._dynamic_batch(input_list)
            results = []
            ...
            for batch in batches:
                ...
                if self.use_quantization:
                    batch_array = self._quantize_batch(batch_array)
                    
                # Process
                batch_result = model(batch_array)
            ...
            # Direct processing
            batch_array = np.stack(input_list) if len(input_list) > 1 else input_list[0]
            ...
            # Process
            results = model(batch_array)
            
            # Dequantize if needed
            if self.use_quantization:
                results = self._dequantize_batch(results)
